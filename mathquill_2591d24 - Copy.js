...
/**
 * MathQuill v1.0.0-alpha.16, by Han, Jeanine, and Mary
...
MathQuill.VERSION = "v1.0.0-alpha.16";
...
Controller.open(function(_, super_) {
  ...
  _.renderLatexText = function(latex) {
    ...
    var mathMode = string('$').then(regex(/^[^\$]*/))
    .skip(string('$').or(eof))// we allow for the case that math mode continues to the end of the stream.
    .map(function(latex) {
...
var TextBlock = P(Node, function(_, super_) {
  ...
  _.latex = function() {
    ...
    return (this.ctrlSeq ==='\\text'&&this.parent?.controller?.KIND_OF_MQ==="TextField")?contents: this.ctrlSeq+'{' + contents + '}';
  };
  _.htmlTemplate='<span class="mq-text-mode">&0</span>';
  _.html = function() {
    return this.htmlTemplate.replace('>&0'," mathquill-command-id='"+this.id+"'>"+this.textContents().replaceAll('<','&lt;').replaceAll('>','&gt;'));
  };
  ...
  _.write = function(cursor, ch) {
    ...
    else if (!cursor[R]) { // at right end of text block
      cursor.insRightOf(this);
...
var RootTextBlock = P(RootMathBlock, function(_, super_) {
  ...
  _.write = function(cursor, ch) {
    ...
    if (ch === '$')
      RootMathCommand(cursor).createLeftOf(cursor);
    else if (cursor[L]?.write) {
      const leftBlock = cursor[L];
      cursor.insAtRightEnd(leftBlock);
      leftBlock.write(cursor, ch);
    } else if (cursor[R]?.write) {
      const rightBlock = cursor[R];
      cursor.insAtLeftEnd(rightBlock);
      rightBlock.write(cursor, ch);
    } else {
      let leftBlock = TextBlock();
      leftBlock.replaces(ch);
      leftBlock.createLeftOf(cursor);
    }
  };
});
...
LatexCmds.cancel = bind(Style, '\\cancel', 'span', 'class="mq-non-leaf mq-cancel"'); 
...
Environments.matrix = P(TabularEnv, function(_, super_) {
  ...
  _.html = function () {
    ...
    this.htmlTemplate =
      ...
      +   (this.environment=='cases'?'>':' style="text-align:center">')
...
Environments.gather = P(TabularEnv, function(_, super_) {
  _.environment = 'gather*';
  _.createBlocks = function() {
    this.blocks = [
      TabularCell(0, this),
      TabularCell(1, this)
  ...
  _.html = function () {
    this.htmlTemplate =
        '<span class="mq-tabular mq-gather mq-non-leaf">'
      +   this.tableHtml()
      + '</span>'
    ;
...
Environments.split = P(Environments.align, function(_, super_) {
  _.environment = 'split';
});
...
Environments.array = P(TabularEnv, function(_, super_) {
  ...
  _.parseAlignment = function(alignment) {
    ...
    var alignmentString = (alignment || "").toString().replace(/[\{\} ]/g, '');
    var results = [];
    var current = {};
